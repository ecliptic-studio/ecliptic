...
// ============================================================================
// ElysiaStreamingHttpTransport
// https://github.com/kerlos/elysia-mcp/blob/main/src/transport.ts
// ============================================================================
...
export class ElysiaStreamingHttpTransport implements Transport {
  ...
  private writeSSEEvent(
    _stream: AsyncGenerator<string | string[]>,
    ...
    try {
      ...
      // Queue the event for streaming (stream parameter not used directly)
  ...
  protected async handleUnsupportedRequest({
    set,
  }: McpContext): Promise<JSONRPCError> {
    set.status = 405;
    set.headers = {
  ...
  private validateSession({ request }: McpContext): {
    valid: boolean;
    status?: number;
      ...
      if (allResponsesReady) {
        if (this._enableJsonResponse) {
          ...
          const responses = relatedIds
            ...
            .filter((response): response is JSONRPCMessage => response !== undefined);
  ...
  private logMessage(message: JSONRPCMessage) {
    if ('method' in message) {
      this.logger.log(
        `method: ${message.method} ${message.params ? 'params: ' + JSON.stringify(message.params) : ''
        }`
      );
    }
  }
...
class Logger {
  ...
  debug(...args: any[]): void {
    if (this.enabled) {
      console.debug('[MCP Transport]', ...args);
    }
  }
}
