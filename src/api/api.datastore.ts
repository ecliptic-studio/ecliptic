...
export const apiDatastore: Partial<Record<Serve.HTTPMethod, Serve.Handler<BunRequest<'/api/v1/datastore'>, Server<undefined>, Response>>> = {
  POST: async (req, server) => {
    ...
    const [result, error] = await createDatastoreController({ session: auth.session, db: kysely }, { provider: validatedBody.data.provider, internalName: validatedBody.data.internalName });
