...
import { listMailboxesController } from '@server/controllers/ctrl.mailbox.list'
...
export const apiMailbox: Partial<Record<Serve.HTTPMethod, Serve.Handler<BunRequest<'/api/v1/mailbox'>, Server<undefined>, Response>>> = {
  GET: async (req, server) => {
		const auth = await resolveAuth(req.headers)
		const lang = resolveLang(req.headers)
		if (!auth) return Response.json({error: 'Unauthorized'}, {status: 401})

		const [result, error] = await listMailboxesController({
