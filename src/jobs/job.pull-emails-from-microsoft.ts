...
export async function jobPullEmailsFromMicrosoft(portal: { db: TKysely }, args: { job: TJob<'pull-emails-from-microsoft'> }): Promise<TErrTuple<TJobResult<Selectable<MailboxEmail>[]>>> {
  try {
    ...
    // dispatch next job, wait 5 minutes if there are no more emails
    const newJobs: Omit<TJob<'pull-emails-from-microsoft'>, 'id'>[] = [{
      ...
      attempts: 0,
      max_attempts: 5,
      runs_after: mails.hasMore ? new Date(Date.now()).toISOString() : new Date(Date.now() + 1000 * 60 * 5).toISOString(),
      payload: {
    ...
    return [null, createError(ErrorCode.JOB_PULL_EMAILS_FROM_MICROSOFT_FAILED)
      ...
      .external({ en: 'Failed to pull emails from Microsoft', de: 'Fehler beim Abrufen der Mails von Microsoft' })
      .statusCode(500)
      .buildEntry()];
  }
